...
Para comparar la eficiencia de \textbf{GA} y \textbf{HC}, utilizamos la misma 
función objetivo (\textbf{GE} o \textbf{RC}) y medimos el tiempo de ejecución 
en los 12 casos de estudio. A partir de estos resultados obtuvimos los siguientes 
tiempos promedios:

\begin{itemize}
  \item \textbf{GA+GE}: 259 segundos
  \item \textbf{HC+GE}: 19.6 segundos
  \item \textbf{GA+RC}: 1809.5 segundos
  \item \textbf{HC+RC}: 622.8 segundos
\end{itemize}

Estos valores muestran con claridad que \textbf{HC} supera sistemáticamente a 
\textbf{GA}. En particular, bajo \textbf{GE} el tiempo de \textbf{HC} es 
aproximadamente 13 veces menor que el de \textbf{GA}, mientras que bajo 
\textbf{RC} es casi 3 veces menor.
...
tiempos promedio indica que \textbf{HC} es aproximadamente \(13.2\times\) más rápido que \textbf{GA}. Bajo \textbf{RC},
\textbf{HC} es \(2.91\times\) más rápido que \textbf{GA}. A nivel de casos
individuales, \textbf{HC} supera a \textbf{GA} en todos los casos con \textbf{GE} y en 11 de 12 casos con \textbf{RC},
siendo \emph{HashSet} la única excepción donde \textbf{GA+RC} fue más veloz. \cacho{Revisar esto, no me acuerdo bien, ver el paper}
Esta diferencia se explica por la naturaleza de los algoritmos: HC sigue una estrategia incremental y determinista que explora de forma más directa el espacio de soluciones, 
mientras que GA introduce variabilidad y requiere manejar poblaciones, lo que aumenta el costo computacional. 
Así, HC resulta sistemáticamente más rápido, aunque GA podría ofrecer más diversidad de soluciones en problemas donde la exploración amplia sea crítica.
...
Esto respalda el uso de GE cuando el objetivo es minimizar el tiempo de cómputo de los generadores.
