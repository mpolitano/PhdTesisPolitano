...
\cacho{La tabla ya fue cambiada, revisar el texto. Hashset estaba cambiado lo corri y evidentemente estaba mal.}
...
Para comparar la eficiencia de \textsf{GA} y \textsf{HC}, utilizamos la misma 
función objetivo (\textsf{FGE} o \textsf{FRC}) y medimos el tiempo de ejecución 
...
\cacho{Estos valores tambien ya fueron actualizados}
\begin{itemize}
...
Estos valores muestran con claridad que \textsf{HC} es sistemáticamente más rápido
que \textsf{GA}. En particular, con \textsf{FGE} el tiempo de \textsf{HC} es 
aproximadamente 13 veces menor que el de \textsf{GA}, mientras que usando 
\textsf{FRC} es casi 3 veces menor. 
\cacho{Cambie aca}
A nivel de casos
individuales, \textsf{HC} supera a \textsf{GA} en todos los casos tanto con \textsf{FGE} y con \textsf{FRC}
...
Comparando las funciones objetivo, \textsf{FGE}
es más rápida que \textsf{FRC}. En \textsf{GA}, FRC requiere en promedio
unas siete veces más tiempo que \textsf{FGE}; en \textsf{HC}, \textsf{FRC} requiere unas casi 32 veces
más tiempo que \textsf{FGE}. La función objetivo \textsf{FGE} debido a que para
computar MGO en nuestros casos de estudio es suficiente con generar objetos de tamaño 
relativamente pequeño (scope 5). Además, \textsf{FGE} utiliza BEAPI (\cacho{ver
...
En conclusión, \textsf{HC+FGE} computa generadores de objetos muy rápidamente en
nuestros experimentos, en 20 segundos en promedio. \textsf{GA+FGE} y
\textsf{HC+FRC} son razonablemente rápidos para esta tarea, 
tomando en promedio poco más de 4 minutos y 10 minutos, respectivamente.
La combinación de \textsf{GA+FRC} hace que el cómputo se vuelva mucho más lento.
...
Se observan cuatro patrones claros. Primero, \textsf{HC+FRC} encuentra el conjunto
minimal exacto en 12/12 sujetos. Segundo, \textsf{HC+GE} es exacto en
11/12 sujetos y sólo sobredimensiona en \textsf{Lits} (retorna 7 en lugar de
...
\emph{Lits} (7 vs.\ 6) y \texttt{HashMap} (2.60 vs.\ 2). Cuarto, \textsf{GA+FGE} es
...
\cacho{
Se observan tres patrones claros. Primero, \textsf{HC+FRC} y \textsf{HC+FGE} encuentra el conjunto de métodos generadores,
minimal, para las doce clases analizadas.
Segundo, \textsf{GA+FGE} obtiene resultados muy cercanos al \emph{minimal} en la mayoría de los casos, 
con algún método de mas en algunas clases (\texttt{NCL}, \texttt{TreeMap}, \texttt{TreeSet}, \texttt{HashSet}).
Por último, \textsf{GA+FRC} muestra un comportamiento correcto pero con un costo computacional considerablemente mayor, 
por lo que no resulta competitivo en términos de eficiencia.
}

\cacho{Revisar esto, cambie la tabla, arregle lits (tiene pinta de TO) y treemap, me da alguna vez algun método de mas, se mete un método que se usa para cubrir código que no es importante}
Estas desviaciones tienen dos causas principales. Por un lado, en \textsf{GE} la
...
incorporar alguna rutina adicional (como ocurre en \texttt{Lits}), lo que hace que
...
En el caso particular del algoritmo genético (\textsf{GA}), además de la
...
\textsf{HC+FGE} además logra el mínimo exacto de forma sistemática. Cuando
el objetivo primario es minimizar el costo de generación/análisis, recomendamos
emplear el conjunto devuelto por \textsf{HC+FGE} (minimal y suficiente).
...
(denominamos \textsf{API} a esta técnica de generación de objetos), y compararla
...
generar tests (llamamos \textsf{MGO} a este enfoque). 
...
En promedio para todos los presupuestos, \textsf{R-Serialize-MGO} genera 1347505 objetos 
frente a 880601 de \textsf{R-Serialize-API}. Es decir, el uso de métodos generadores resulta
en un incremento de 1.53 veces en la cantidad de objetos distintos producidos. 
\cacho{Actulice valores}
Desagregando por presupuesto, para \textsf{60 segundos} con métodos generadores 
se obtienen, aproximadamente, dos veces más objetos, que con toda la API. 
Con un presupuesto de \textsf{120 segundos}, se generan aproximadamente 1.5 
veces más objetos con los \texttt{MGO} en comparación con la \textsf{API},
Finalmente, para \textsf{180 segundos}, la configuración con métodos 
generadores se construyen casi 1.37 veces más objetos con los \textsf{MGO} en
comparación con la \textsf{API}. Esta diferencias es debido a que, al aumentar el presupuesto, Randoop tiene más tiempo para explorar la API completa y generar más objetos,
...
\textsf{MGO} ejerciten mejor el código que los generados por
\textsf{API} si se los utiliza para testing. 
La calidad de los objetos generados se evalúa más adelante en la siguiente Sección \ref{sec:experiments-builders-parameterized}, 
donde los utilizamos como entradas de tests parametrizados sobre las estructuras de datos. 

\subsubsection{Testing parametrizado}
...
herramientas utilizadas para generar estos objetos fueron: la versión estándar de \textsf{Randoop}, que 
genera directamente una suite de tests sin reutilizar objetos previos; una variante de \textsf{Randoop} 
denominada \textsf{R-Serialize-API}, que serializa los objetos construidos durante la generación de 
tests, permitiendo su reutilización como entrada en tests parametrizados; una versión modificada 
de \textsf{Randoop} que prioriza el uso de métodos generadores de objetos identificados previamente mediante 
nuestro enfoque (denominada \textsf{R-Serialize-MGO}, ver sección \ref{sec:object_builders_randoop}); 
...
del paquete \texttt{java.util} debido a su rica API que tienen: \texttt{HashSet}, \texttt{HashMap}, \texttt{TreeMap}, \texttt{TreeSet} y \texttt{LinkedList}.
Además, para poder utilizar todos los \emph{benchamark} se necesita escribir test parametrizados para todos, siendo esta, una tarea laboriosa.
Las métricas consideradas fueron: el tiempo 
de generación (\textsf{GTime}), este tiempo es el que se le da cada herramienta para ejecutar.
Además se tuvo en cuenta las métricas de la cantidad de casos de prueba ejecutados (\textsf{Test}), el 
tiempo total de ejecución de la test suite (\textsf{T(Seg)}), la cobertura de ramas alcanzada 
(\textsf{Ramas}) y la cantidad de mutantes eliminados (\textsf{Mutacion}). Es importante destacar 
que la test suite parametrizada es común a los enfoques \textsf{R-Serialize}, \textsf{R-Builders}, permitiendo así una comparación justa sobre los objetos generados. En el caso de 
\textsf{Randoop}, la herramienta genera su propia test suite, por lo que los resultados deben 
interpretarse con cautela.


\input{tables/randoopvsBuilders}

Las tablas anteriores, demuestran que los resultados obtenidos con \textsf{R-Serialize-MGO} son consistentemente superiores a los alcanzados por \textsf{R-Serialize-API} y \textsf{Randoop} en todos los casos evaluados. 
\textsf{R-Serialize-MGO} mejora la cobertura de ramas y mutación con sus objetos generados a partir de los métodos generadores identificados previamente.
...
Además, podemos afirmar que los MGO son útiles para mejorar las herramientas de testing automático como Randoop.


\cacho{Explico tabla por tabla?}


\begin{lstlisting}[caption={Ejemplo de Test Parametrizado de la clase \texttt{LinkedList}},label={lst:parametrizadoLinkedList},language=Java,captionpos=b]
public void addTest() {			
    ...
    while(it.hasNext()) {
        ...
        int oldS = l.size();
        boolean res=l.add(i);
        assertTrue(l.size() == oldS+1 && res || l.size() == oldS && !res);
    }
}
...
public void removeTest() {
    ...
    while(it.hasNext()) {
        ...
        size = l.size();
        int i = ThreadLocalRandom.current().nextInt(0, literals + 1);
        int indexToRemove = l.indexOf(i);
        containsPrevRemove = l.contains(i);
        res = l.remove(i);
        assertTrue((res && containsPrevRemove ) || !res && !containsPrevRemove) ;
        assertTrue(size-1 == l.size() && res || size == l.size() && !res );
    }
}
...
\cacho{Nuevo}Los métodos generadores de objetos fueron precomputados usando el algoritmo de \textsf{Hill Climbing} (\ref{alg:approachHC}) 
con la función objetivo que utiliza los objetos que produce el generador exhaustivo acotado (\ref{sec:fitnessGE})
...
por el estilo?} \cacho{Lo sacamos, no hay nada de esto, la idea era como decir que no se pierden estructuras, que se aseguran que están las mismas y mas}
...
antes del timeout, estas 60 combinaciones, el \emph{driver} con \textsf{MGO} explora 14070756 estados
en 1774 segundos, mientras que \textsf{API} visita 198130649 estados en 17240 segundos.
En promedio por casos, \textsf{MGO} explora 234513 estados en 29.5 segundos, mientras que \textsf{API} 
explora 3302177 estados en 287.3 segundos. Esto implica que \textsf{MGO} reduce el espacio de estados en 
92.9\%\ y el tiempo en 89.7\%\ aproximadamente respecto de \textsf{API}. 
% Se observa tambien que \textsf{API} alcanza el timeout en 5/60 combinaciones
% (8\%) \pp{son más que 9, hay que recalcular los números?}. 
...
\texttt{FAJITA} \cite{Abad13}, \texttt{ROOPS} \cacho{VER CITA}. También
...
\textsf{Hill Climbing} con la función objetivo basada en el generador exhaustivo acotado. (ver Sección: \ref{sec:eficienciaBuilders})
...
no se dispone de \texttt{repOK}s para estas clases, lo que impide una comparación justa con \texttt{Korat}.
...
en \texttt{FAJITA}, \textsf{BEAPI} supera a \textsf{Korat} en 3 de 4 casos; en
\texttt{ROOPS}, \textsf{BEAPI} lo hace en 5 de 7 casos; 
y en \texttt{Kiasan}, \textsf{Korat} es más rápido en 6 de 7 clases.
...
La Tabla \ref{tab:results-realWorld} resume los resultados para el conjunto \texttt{Real World} \pp{Y las
...
estructuras del benchmark \texttt{Real World} \pp{Hay una mezcla de fuentes
...
En esta sección, evaluamos la calidad de los objetos generados por BEAPI en el contexto del testing parametrizado.
...
mutación equivalente a \textsf{R-Serialize-MGO}, y superior a la alcanzada por \textsf{R-Serialize-API} y 
\textsf{Randoop} en lapsos de tiempo cortos. Esta eficiencia evidencia que BEAPI puede generar un 
...
\textsf{BEAPI} alcanza una cobertura de ramas y mutación mejor en cuanto a mutantes matados con respecto a las otras técnicas.
En el caso de la tabla \ref{tab:treeMapTools_beapi}, se observa que se mantiene la tendencia de que los objetos generados por BEAPI son efectivos para testing parametrizado, logrando coberturas competitivas con tiempos de generación muy bajos.
En este caso, \textsf{BEAPI} alcanza una cobertura de ramas y mutación superior pero se necesitaron 30 segundos, lo cual sigue siendo un tiempo bajo ya que la mejor técnica (\texttt{R-Serialize-MGO}) necesita 180 segundos para lograr una cobertura similar.
...
\texttt{LinkedList}. En este caso, se observa nuevamente que \textsf{BEAPI}, a pesar de 
generar un número reducido de objetos, mantiene niveles constantes de cobertura y mutación. 
Las técnicas \textsf{R-Serialize-API} y \textsf{R-Serialize-MGO} logran cubrir las mismas ramas y casi los mismos 
mutantes que \textsf{BEAPI}, esto se debe a que las listas enlazadas son estructuras más simples,
...
similar. \textsf{BEAPI} logra altos niveles de cobertura con tiempos de generación de 89 segundos, para alcanzar el máximo de cobertura que se puede obtener.
En este caso \textsf{R-Serialize-MGO} se acerca a ese valor pero no llega al mismo en 180 segundos.
...
Randoop mediante la identificación previa de métodos generadores de objetos también mejora significativamente 
...
Este conocimiento no solo permite mejorar herramientas existentes como Randoop (caso de \textsf{R-Builders}), 
...
\texttt{LinkedList}. En este caso, se observa nuevamente que \textsf{BEAPI}, a pesar de 
generar un número reducido de objetos, mantiene niveles constantes de cobertura y mutación. 
Las técnicas \textsf{R-Serialize} y \textsf{R-Builders} logran cubrir las mismas ramas y casi los mismos 
mutantes que \textsf{BEAPI}, esto se debe a que las listas enlazadas son estructuras más simples,
donde la mayoría de las configuraciones válidas pueden alcanzarse mediante secuencias cortas
de inserciones y eliminaciones..
...
correspondiente a \texttt{HashMap}, se aprecia un caso 
similar. \textsf{BEAPI} logra altos niveles de cobertura desde tiempos de generación muy 
reducidos, mientras que las otras técnicas necesitan un volumen considerable de objetos y 
tests para alcanzar resultados comparables. En este caso \textsf{R-Builders} obtiene un puntaje de mutación 6–7\% mayor que \textsf{BEAPI} (127–128 vs 119–120). 
Una explicación es que varios mutantes exitosos están asociados a rutas de código y estructuras intermedias que \textsf{BEAPI} no utiliza:
por ejemplo, secuencias largas con inserciones/eliminaciones que pasan por estados transitorios (similar a los casos de \texttt{TreeMap} y \texttt{Treeset}). 
\textsf{R-Builders}, al explorar más operaciones y estados no canónicos, tiende a tocar esos caminos y matar esos mutantes extra, aunque al costo de mucha más ejecución. 
De hecho, en ramas \textsf{BEAPI} iguala o supera a \textsf{R-Builders} ( 122 vs 119 a partir de 
GTime=10), reforzando la idea de que la ventaja de \textsf{R-Builders} en mutación proviene de probar más tipos de operaciones y estados que por cubrir mejor el núcleo de la estructura.
