...
Politano24}.
En BEAPI la identificación de métodos generadores de objetos se
realiza en una etapa previa a la generación de objetos, usando el algoritmo
\pp{Poner cual de los algoritmos de la tesis usamos. Charlemoslo}. 
Para esta etapa, se empléo un scope de 5 en todos los casos, y el tiempo máximo de ejecución del 
enfoque de identificación fue de 65 segundos en los benchmarks \textsf{Kiasan},
\textsf{FAJITA}, \textsf{ROOPS} y \textsf{Korat} (para el caso SLL de ROOPS, con 11
métodos), y de 132 segundos en \textsf{Real World} (para TreeMap, que tiene 32
métodos). Verificamos manualmente que los métodos identificados incluyeran un
conjunto suficiente de métodos generadores de objetos en todos los casos.
...
\cite{Boyapati:2002} \pp{Buscar la cita de esto en el paper de beapi}
...
tesis. Chequear en toda la tesis.} que habilita tanto 
...
La Tabla \ref{tab:results-realWorld} \cacho{Ver como referencias, hacer una sola
tabla} resume los resultados para el conjunto \textsf{Real World} \pp{Y las
...
scopes significativamente mayores en todos los casos excepto TreeMap y TreeSet
\pp{se llaman distinto en la tabla}, donde igualmente \textsf{CI/MGO} muestra
...
Clasificamos estos errores como \texttt{under} \pp{Yo lo reemplazaría por sub,
así queda en castellano},
...
que fueron evidenciados por estas estructuras como \texttt{over} \pp{Yo lo
    reemplazaría por sobre, así queda en castellano}.
...
y puedo haber empeorado el formato}
