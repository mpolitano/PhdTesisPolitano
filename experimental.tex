...
de Politano et. al. \cite{} \pp{Tu paper de builders} presenta el algoritmo
...
cada uno, por función objetivo (FGE y FRC) \pp{Actualizar la tabla con estos
nombres}. Para cada configuración reportamos el promedio de 5 ejecuciones.
...
Estos valores muestran con claridad que \emph{HC} es sistemáticamente más rápido
que \emph{GA}. En particular, con \emph{FGE} el tiempo de \emph{HC} es 
aproximadamente 13 veces menor que el de \emph{GA}, mientras que usando 
\emph{FRC} es casi 3 veces menor. 
...
El algoritmo genético (\textbf{GA}) mantiene y evoluciona poblaciones de
candidatos, lo que implica evaluar la función objetivo (que es costosa
computacionalmente) para muchos candidatos.
En cambio, \emph{HC} \pp{Hay que poner en typewriter la fuente de los algoritmos
y casos de estudio en todo el texto. emph no se usa para eso, sino para resaltar
palabras, y poner palabras en inglés} sigue una estrategia \emph{greedy}: elige la mejor opción posible entre los sucesores del candidato actual. Esto lo vuelve más eficiente.
Sin embargo, en ciertos casos \emph{HC} podría perder precisión en el cómputo de métodos
generadores. 

Comparando las funciones objetivo, \emph{FGE}
es más rápida que \emph{FRC}. En \emph{GA}, FRC requiere en promedio
unas siete veces más tiempo que \emph{FGE}; en \emph{HC}, \emph{FRC} requiere unas casi 32 veces
más tiempo que \emph{FGE}. La función objetivo \emph{FGE} debido a que para
computar MGO en nuestos casos de estudio es suficiente con generar objetos de tamaño 
relativamente pequeño (scope 5). Además, \emph{FGE} utiliza BEAPI (ver
sección \ref{} \pp{referencia}), que implementa mecanismos que hacen a la
generación acotada más eficiente (generación guiada por el feedback de la
ejecución, state matching). 
Todas \pp{Ambas funciones?} tienen un \emph{timeout} de 
...
cobertura sino que es aleatoria).
Esto explica porque FGE es más rápida en comparación con la función objetivo FRC.

Por otra parte, la función objetivo \emph{FRC} no requiere que el usuario provea
la definición del scope para la generación exhaustiva acotada. Además, \emph{FRC}
podría ser más escalable que \emph{FGE} en casos donde
...
que los evaluados aquí). Esto se debe a que \emph{FRC} evalúa cada candidato
mediante generación de tests sin explorar exhaustivamente el
espacio de objetos.
Como trabajos futuros se planean realizar más experimentos para evaluar la
utilidad de \emph{FRC} en este tipo de casos \pp{No olvidarse de incluir esto en
trabajos futuros}.

En conclusión, \texttt{HC+FGE} computa generadores de objetos muy rápidamente en
nuestros experimentos, en 20 segundos en promedio. \texttt{GA+FGE} es
razonablemente rápido para esta tarea, tomando en promedio poco más de 4 minutos.
...
objetos para testing, etc.), creemos que tanto \texttt{HC+FGE} como \texttt{GA+FGE} 
son algoritmos viables para su uso en la práctica. 

Los resultados preliminares muestran que con la función objetivo
\texttt{FRC} los algoritmos son mucho más lentos. Se deja como trabajo futuro
estudiar formas más eficientes de implementar la función \texttt{FRC}.
...
\subsubsection{Impacto en verificación acotada usando JPF}
...
de la verificación acotada usando \emph{Java PathFinder}
...
\pp{Tengo que modificar que esto se trata ahora en la sección de builders.} 
...
y el artefacto experimental con el código fuente se encuentra disponible en: \cacho{Poner pagina?}
...
(seis implementaciones ampliamente usadas: \texttt{LinkedList} con 67 métodos, \texttt{TreeSet} con 34 métodos, \texttt{TreeMap} con61 métodos, \texttt{HashMap} con 45 métodos, \texttt{NCL} con 34 métodos y \texttt{Schedule} con 12 métodos). 
