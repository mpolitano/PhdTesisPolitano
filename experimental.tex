...
objetivo, \textbf{GE} y \textbf{RC}.
Para cada configuración (algoritmo $\times$ función objetivo) reportamos el tiempo
promedio en segundos sobre 5 ejecuciones.


Analizando la Tabla~\ref{tab:eficiencia}, observamos que \textbf{HC} es
generalmente más eficiente que \textbf{GA} para
ambas funciones objetivo, \textbf{GE} y \textbf{RC}.
Para hacer este analisis podemos considerar primero un analisis entre algortimo, con la misma funcion objetivo:
Tomando el \emph{speedup} como \(S=\tfrac{t_{\text{GA}}}{t_{\text{HC}}}\):
\begin{itemize}
  \item \textbf{GE:} HC es \(\mathbf{13.1\times}\) más rápido que GA (reducción de tiempo \(\approx\)\(\mathbf{92.4\%}\)).
  \item \textbf{RC:} HC es \(\mathbf{2.91\times}\) más rápido que GA (reducción de tiempo \(\approx\)\(\mathbf{65.6\%}\)).
\end{itemize}

Ademas podemos comparar \textbf{GE} vs. \textbf{RC} para cada algoritmo:
\begin{itemize}
  \item \textbf{GA:} GE es \(\mathbf{7.0\times}\) más rápida que RC (RC requiere \(\sim\)86\% más tiempo).
  \item \textbf{HC:} GE es \(\mathbf{31.5\times}\) más rápida que RC (RC requiere \(\sim\)96.8\% más tiempo).
\end{itemize}
...
En relación con las funciones objetivo, observamos que la basada en
...
basada en cobertura con Randoop (\textbf{RC}, Sección~\ref{sec:fitnessRandoop}), independientemente del algoritmo usado.
...
En promedio, \textbf{HC} domina a \textbf{GA} para ambas funciones objetivo, con una
ventaja especialmente marcada bajo \textbf{GE} (\(13.1\times\)). A su vez, \textbf{GE} resulta
sistemáticamente más eficiente \cacho{CUanto?} que \textbf{RC} dentro de cada algoritmo, lo cual es consistente
con el mayor costo de evaluación de RC (generar/ejecutar suites con Randoop y medir
cobertura) frente a la evaluación por enumeración acotada de GE.
...
Promediando sobre los 12 casos se obtiene: \(\text{GA+GE}=259\) s,
\(\text{HC+GE}=19.6\) s, \(\text{GA+RC}=1809.5\) s y \(\text{HC+RC}=622.8\) s.
...
tiempos promedio indica que HC es aproximadamente \(13.2\times\) más rápido que GA,
lo que equivale a una reducción de tiempo cercana al \(92.4\%\). Bajo \textbf{RC},
HC es \(2.91\times\) más rápido que GA (reducción del \(65.6\%\)). A nivel de casos
...
más tiempo que GE. Este comportamiento es coherente con la naturaleza de cada
función: GE evalúa candidatos por enumeración acotada (scope fijo), mientras que RC
debe, para cada candidato, generar y ejecutar suites con Randoop y medir cobertura,
lo que incrementa el costo de evaluación.
...
El costo de \textbf{GE} escala con la enumeración de \emph{todas}
las instancias dentro del scope, mientras que \textbf{RC} evalúa cada candidato
mediante generación/ejecución de tests \emph{sin} explorar exhaustivamente el
espacio de objetos, lo que puede resultar más económico para scopes grandes.

% Formalmente, si denotamos por $E(n)$ la cantidad de objetos válidos bajo scope $n$
% y por $B(n)$ el presupuesto (número de tests $\times$ longitud) para RC, cuando
% $E(n)$ crece exponencialmente y $B(n)$ se mantiene acotado o crece a ritmo
% polinomial, existe un $n_0$ tal que para todo $n \ge n_0$ el costo de evaluar un
% candidato con \textbf{RC} será menor que con \textbf{GE}. En otras palabras, si
% $E(n) \gg B(n)$, la evaluación por cobertura tiende a ser más escalable que la
% enumeración acotada.
...
emplear el conjunto devuelto por \textbf{HC+RC} (minimal y suficiente); cuando el
objetivo es maximizar la robustez frente a restricciones operativas del
\emph{scope} o de la cobertura, \textbf{RC} ofrece estabilidad en la suficiencia,
aun al costo de incluir ocasionalmente métodos adicionales.
...
pruebas: 60, 120 y 180 segundos. Los resultados obtenidos se resumen en la
Tabla~\ref{tab:results-obj}.

En promedio global sobre las 39 combinaciones (clase X presupuesto),
\texttt{SoloMetodosGeneradores} genera \textbf{6355} objetos frente a \textbf{1597} de
\texttt{TodosLosMetodos}, es decir, \textbf{+4758} objetos en promedio y un
incremento de \textbf{\(\sim 4.0\times\)} (razón de medias
\(6355/1597=3.98\)). Desagregando por presupuesto:
...
permite obtener \textbf{entre \(3.75\times\) y \(4.09\times\) más objetos} en
promedio, con una ganancia absoluta que crece al aumentar el tiempo
presupuestado.

\cacho{Ver caso Scheduler, porque paso esto, en el paper tampoco esta explicado}
La ventaja es consistente en la gran mayoría de los casos: en \textbf{36 de 39}
...
\emph{Schedule}, donde \texttt{TodosLosMetodos} produce más objetos; además, en
\emph{Lits} la mejora es modesta. En contraste, la ganancia es muy marcada en
\emph{NCL} (\(\sim 15.4\times\) promedio por clase) y \emph{LinkedList}
(\(\sim 10.5\times\)).

En síntesis, \emph{en promedio} el uso exclusivo de métodos generadores
produce \textbf{casi cuatro veces más} objetos que emplear toda la API, y lo
hace de manera robusta en la mayoría de las clases y presupuestos. Esto sugiere
que focalizar la generación sobre operaciones constructivas efectivas mejora la
...
También comparamos las \emph{suites} generadas midiendo cobertura de ramas y líneas.
Como en el experimento anterior, contrastamos \texttt{MetodosGeneradores} contra
\texttt{API} con Randoop por defecto, ahora con cinco presupuestos (60, 120, 180,
300 y 600 s). Los resultados se presentan en la Tabla~\ref{tab:results-coverage}.
En promedio sobre todas las clases y presupuestos, \texttt{MetodosGeneradores} mejora la
\textbf{cobertura de ramas} de \textbf{104{.}0} a \textbf{120{.}9} ramas
(\textbf{+16{.}8} ramas, \textbf{+11{.}8\%}) y la \textbf{cobertura de líneas} de
\textbf{223{.}1} a \textbf{245{.}4} líneas (\textbf{+22{.}2} líneas, \textbf{+7{.}0\%}).
Por presupuesto, las ganancias de ramas crecen con el tiempo
(\(+13{.}9\), \(+15{.}1\), \(+15{.}5\), \(+19{.}5\), \(+20{.}2\) ramas para
60, 120, 180, 300 y 600 s; equivalentes a \(+14{.}0\%\)–\(18{.}7\%\)), mientras que
en líneas se observan mejoras de \(+8{.}97\%\) a \(+10{.}6\%\).
\texttt{MetodosGeneradores} nunca empeora la cobertura de ramas (41/65 mejoras, 24 empates)
...
y \(+30\%\) y \(+34\%\) en líneas), mientras que en APIs pequeñas o ya
saturadas (\emph{BTree}, \emph{Schedule}, \emph{UFind}) las coberturas de
\texttt{API} y \texttt{MetodosGeneradores} son prácticamente idénticas.
...
Bajo el mismo presupuesto, \texttt{MetodosGeneradores} produce \(\sim 4\times\)
...
\input{tables/coverageRandoop.tex}
...
Como ya se mostro anteriormente, ver seccion \ref{sec:jpf}, el primer gran aporte a JPF se
...
de obtener previamente los métodos generadores es más sencillo y entendible para el usuario.

Para ilustrar este mecanismo, consideramos el análisis del método \texttt{put} de \texttt{TreeMap}. \cacho{Como explicar esto, que no es una propiedad como lo explique}
...
métodos generadores permiten una mejora sustancial en eficiencia y escalabilidad
\pp{Agregar promedios a las tablas, y agregar los números promedios para
    justificar las conclusiones abajo.}
. Se observa una 
