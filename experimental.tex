...
existen observadores monitoreando su estado.\pp{No se entiende qué hace Lits}
...
\texttt{addFirst} y \texttt{addLast} en \texttt{NCL}). \pp{el encabezado de la
tabla está en inglés. No usamos la palabra builders acá. Y el caption? No se
entiende que dice.}
...
efectividad. En particular, los parámetros más relevantes para el algoritmo
genético ((Sección~\ref{alg:approachGA})) incluyen la tasa de mutación, la tasa de cruce, el tamaño de la
población y la estrategia de selección utilizada. \pp{Este párrafo está repetido
abajo agregando solo los valores para los parámetros. Por qué no ponerlos
acá directamente?}
...
distintos escenarios. \pp{Este párrafo está repetido
abajo agregando solo los valores para los parámetros. Por qué no ponerlos
acá directamente?}
...
del objeto. \pp{Este párrafo está repetido
abajo agregando solo los valores para los parámetros. Por qué no ponerlos
acá directamente?}
...
el car\'acter aleatorio del generador. Esta configuración permite obtener una medida más robusta del desempeño 
de los algoritmos en contextos con elementos no determin\'isticos.
...
constructores. Además, se ignoraron campos de las clases evaluadas que no
alteran la estructura lógica de los objetos generados, tales como el campo
\texttt{modCount}. \pp{Yo no pondría esto último porque no recuerdo que lo
hayamos explicado en ningún lado.}

Para cada combinación de caso de estudio y función de valoración, el algoritmo
genético fue ejecutado cinco veces con los parámetros mencionados \pp{Y HC}. Los
resultados reportados en las siguientes secciones corresponden al promedio de
estas ejecuciones. En el caso particular de la función de valoración \pp{no
usamos el término función de valoración. Siempre pusimos función objetivo.
Chequeuar en todos lados.} basada en
...
funciones objetivo propuestas \pp{Nombres}. Los resultados se presentan en la Tabla \ref{tab:eficiencia}.
\pp{Corregir el caption de la tabla.}

Como se observa en la tabla, el algoritmo Hill Climbing (\emph{HC}) es más
efectivo \pp{eficiente?} que el algoritmo genético (\emph{GA}) en todos los casos de estudio considerados.
...
el peor de los casos \pp{Cuál?}. \pp{Hay que poner tiempos promedios además del
...
En relaci\'on con las funciones de valoraci\'on \pp{funciones objetivo las
llamamos en toda la tesis. Si la vas a llamar función de valoración hay que
cambiarlo en todos lados}, observamos que la basada en el generador exhaustivo (GE) logra 
finalizar la búsqueda de métodos generadores de objetos en menos tiempo en comparación con la función basada en la 
cobertura generada por la suite de pruebas \pp{No usar más la palabra pruebas.
Es tests. O cambiarla en toda la tesis.} el algoritmo \pp{Poner el nombre del
...
de valoración GE termina antes en los algoritmos debido a que genera objetos
\pp{para un scope que debe ser especificado manualmente por el usuario. } (en este caso, 5 para todos 
...
que \pp{Nombre del algoritmo} más rápida \pp{cuánto?} en comparación 
con la función de valoración \pp{objetivo} basada en Randoop \pp{No es basada en Randoop, es
el algoritmo presentado en la sección correspondiente. Sino le bajás el
precio.}. En cambio, el algoritmo \pp{XX} debe agotar su presupuesto de tiempo
...
Es importante destacar que todos los algoritmos se ejecutan con 10 hilos
simultáneamente, evaluando así 10 candidatos a la vez para cada algoritmo
\pp{Esto se contradice con lo que dice la sección de genéticos}.
...
algoritmo Hill Climbing con una función de valoración que cuente los objetos
...
a identificar el conjunto mínimo \pp{Minimal. Tener en cuenta los términos
precisos que usamos durante toda la tesis.} de métodos 
...
usabilidad de los generadores de objetos para el análisis de programas.
...
(\emph{GE}) y otra basada en la cobertura de ramas obtenida a partir de una
suite de pruebas generada con Randoop (\emph{RC}) \pp{Todo esto ya está dicho
arriba}.
...
diferentes valores dependiendo de la semilla inicial \pp{Esto va en la
definición de los algoritmos arriba. Y hay que explicar como mitigamos este
problema, o sino directamente capaz que no lo mencionamos.}.

En el caso de la estructura \emph{NCL}, el algoritmo \emph{HC} utilizando la
función de valoración \emph{GE} \pp{Creo que le pondría HC+GE, porque sino es
muy repetitivo y largo todo el texto.} logra encontrar el 
conjunto mínimo exacto de métodos generadores en todas las ejecuciones. Sin embargo, cuando se utiliza la función \emph{RC}, 
el mismo algoritmo encuentra en promedio un conjunto con 3.80 métodos \pp{acá el
promedio no es lo mejor, habría que explicar los casos por separado. Pero ya lo
hiciste así que fue. si lo cambiamos va a ser mucho laburo.}, lo que indica que en tres de las cinco ejecuciones se incluyó 
un método adicional no necesario. Por su parte, el algoritmo evolutivo
(\emph{GA}) encuentra en todas las ejecuciones los metodos correctos con ambas
funciones de valoracion \pp{No es lo que dice la tabla}. \pp{El caption de la
tabla hay que mejorarlo.} \pp{La tabla no está referenciada en el texto?}

En estructuras como \emph{UnionFind}, \emph{FibonacciHeap}, \emph{BTree}, \emph{BinomialHeap} y \emph{Scheduler}, todos los 
algoritmos evaluados lograron encontrar el conjunto mínimo exacto de métodos
\emph{builders} \pp{No usar builders.}. Esto puede atribuirse a que estas 
...
se logra encontrar el conjunto mínimo real \pp{Y por qué no hicimos esto en
lugar de que calcule mal? No entiendo bien el problema, hay que charlarlo.} En contraste, la función \emph{RC} encuentra siempre el conjunto mínimo, dado que su 
...
\pp{Agregar una subsubsection para separar el experimento de tamaño de objetos generados
con Randoop, y otra para el experimento de bounded model checking con JPF.}
...
Tabla~\ref{tab:results-obj}. \pp{Agregar promedios a las tablas, y agregar los números promedios para
