...
\texttt{LinkedList}},label={lst:parametrizadoLinkedList},language=Java,captionpos=b]
...
\end{lstlisting}
\pp{Quedó cortado el listing, arreglar!}
...
\pp{En las tablas hay algunos errores. Faltan acentos en varias palabras. Para randoop poner numtests/-- como
indicando que Obj no aplica. Hay una columna que dice Scope que no va}


En \texttt{LinkedList} \pp{referencia a la tabla}, la estructura más simple de todas las
consideradas, \texttt{RSer-MGO} y \texttt{RSer-API} maximizan cobertura en
60 segundos de generación. \texttt{RSer-MGO} obtiene el máximo puntaje de mutación
en 120 segundos, mientras que \texttt{RSer-API} lo hace en 180 segundos.
...
En \texttt{HashSet} \pp{referencia a la tabla}, \texttt{RSer-MGO} y \texttt{RSer-API} maximizan cobertura en
120 segundos. \texttt{RSer-MGO} obtiene el máximo puntaje de mutación
en 120 segundos, mientras que \texttt{RSer-API} nunca llega a matar la máxima cantidad de mutantes.
En \texttt{HashMap} \pp{referencia a la tabla},
\texttt{TreeMap} \pp{referencia a la tabla} y \texttt{TreeSet} \pp{referencia a
la tabla}, las mejoras en cobertura y mutantes muertos por parte de \texttt{RSer-MGO} son
significativas para todos los tiempos de generación. Además, \texttt{RSer-MGO}
siempre logra los máximos puntajes, mientras que \texttt{RSer-API} no logra los
máximos puntajes ni siquiera para el máximo tiempo de generación (180 segundos).
