...
En esta sección, evaluamos la calidad de los objetos generados por BEAPI en el
contexto del testing basado en propiedades.
Para ello, comparamos BEAPI con los enfoques de generación aleatoria utilizados
...
Como la generación con BEAPI se lleva a cabo por scope (y no por tiempo como en
los otros enfoques), incluimos en los resultados el scope para el que BEAPI
termina dentro de cada presupuesto de generación predefinido. Así, BEAPI-sXX denota
la técnica BEAPI ejecutada para un scope de XX. \pp{son raros los números
    redondos en BEAPI. Por ejemplo, veo en HashSet scope 14 que da 50000 y
700000 redondos. Me dan desconfianza esos números. Si vamos a redondear, habría
que reportar algo como 50k objetos, 700k tests y hacer eso para todas las
técnicas. Lo podemos que charlar.}

\input{tables/randoopvsBeapi}

\pp{Pablo: Revisar desde acá}
A continuación se describen los resultados de este experimento. 
La Tabla \ref{tab:hashSetTools_beapi} muestra una comparativa de diferentes
técnicas aplicadas para \texttt{HashSet}. \textsf{BEAPI} logra una cobertura de ramas y 
mutación equivalente a \textsf{RSer-MGO}, y superior a la alcanzada por \textsf{RSer-API} y 
...
\pp{Pablo: Revisar hasta acá}

Los resultados obtenidos permiten concluir que los objetos generados por \textsf{BEAPI} son 
efectivos para el testing basado en propiedades. Si bien usar scopes grandes
puede ser prohibitivo (debido a la exhaustividad de la técnica), en general, usar scopes
pequeños es suficiente para lograr maximizar cobertura y mutación. 
