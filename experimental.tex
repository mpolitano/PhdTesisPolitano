...
\cacho{Este experimento no se si agregarlo, lo saque del paper. No es tan asi, y capaz que traiga problema con el experimento de beapi. Lo mismo que los objectos de arriba, no coinciden los valores. Ver con PP}
...
En cada configuración, construimos las \emph{suites} de prueba ya sea utilizando 
...
mas conocimento de la API bajo análisis como muestra el algoritmo \ref{lst:driverNCL}. Para
compensar esto, mostramos que el algoritmo que se utiliza como controlador en caso
de obtener previamente los métodos generadores es más sencillo y entendible para el usuario (Ver \ref{sec:jpf-builders}).
...
. Se observa una reducción considerable en el tiempo de análisis y en la cantidad de estados explorados, sin 
...
\input{tables/tableJPFBuilders_short.tex}
