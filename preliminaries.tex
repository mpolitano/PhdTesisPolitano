...
\pp{El ejemplo está bien, pero está desconectado de la explicación de arriba y
    del algoritmo de la Figura 1. Por ejemplo, acá abajo habla de un pool que no 
    existe en el algoritmo. Hay que relacionar cada paso de la ejecución de
    abajo con las líneas del algoritmo de la Figura 1.}

    
Para ilustrar el funcionamiento de Randoop, consideremos la clase
\texttt{java.util.LinkedList}. 
\cacho{Esto es nuevo:}
El generador comienza con un conjunto inicial de valores primitivos (por ejemplo, 0, 1, 2, –1, –2) (\cacho{Lo agrego en el algoritmo esto?}) 
y los métodos públicos disponibles. En una primera ejecución, Randoop
...
para instanciar, se saltea la parte de las líneas 5 a 12. Luego ejecuta la secuencia (línea 14):
\\
...
se incorporan al \emph{pool} para futuras extensiones (\texttt{prev} en el algoritmo).

En una iteración posterior, Randoop elige al azar un método, por ejemplo,
\texttt{add(Object)} (línea 4). Para obtener una lista sobre la cual invocar el
método, reutiliza la secuencia previa que construye \texttt{l} (\cacho{Esto no esta muy claro en el algortimo}). Para el
parámetro, como es de tipo primitivo, toma un entero disponible en el conjunto inicial de valores primitivos (por ejemplo, 1).
La secuencia extendida queda (línea 13):
...
La ejecución es exitosa (línea 14) y la nueva secuencia pasa a estar disponible para
nuevas extensiones (Línea 22).
...
excepción por índice fuera de rango (línea 14). Debido a su estrategia de
\emph{feedback-directed random testing}, Randoop descarta esa secuencia
fallida y deja de ampliarla (línea 16). Solo las secuencias que ejecutan sin fallos
...
\end{lstlisting}
\cacho{cambie a listing de java que es como esta el driver}
\pp{Están feas las fuentes del algoritmo. En general el código va en texttt. 
Acá hay una mezcla rara de cursivas con tt. Por qué es distinta la fuente de
este programa que del driver?}

El Algoritmo \ref{lst:propiedad} muestra un ejemplo de propiedad a
...
listas de tamaño máximo \(b\), con elementos en \([0,b-1]\) \pp{Si son b nodos,
entonces son enteros entre 0 y b-1. No hay ninguna b en el código. Hay que
revisar el texto de esta sección actualizando las variables por las del
código.}. \cacho{Aca quería explicar, hacer una intro por eso no esta bien. Creo que lo puedo sacar porq es algo redundante con el parrafo de abajo} Así, las entradas generadas por el
\emph{driver} son usadas por JPF para la verificación de la propiedad.

Un driver básico para la propiedad del Algoritmo \ref{lst:propiedad} se
muestra en la Figura \ref{lst:driverAPI} \pp{El driver tiene que coincidir con
la propiedad de listas, no me sirve el driver de árboles para este ejemplo}. \cacho{Cambie el driver a listas}
...
tamaño hasta \(scope-1\). Luego, el driver procede a la ejecución de \texttt{maxLength} 
métodos de la API (línea 4). A cada operación se le asigna un
\pp{No existe m en el algoritmo. por otro lado no cierra que sea en 0..m-1,
porque la definición de getInt arriba dice que es en 0..m. Hay que ver cual es
el correcto y actualizar en toda la sección, y en la sección de builders}
\cacho{Es hasta m, https://wuyongzheng.github.io/jpfdoc/gov/nasa/jpf/vm/Verify.html. Lo saco lo de m, lo habia puesto como para explicar pero no se entiende}
identificador único, y se elige no determinísticamente (\texttt{n\_methods}) para
seleccionar la operación a ejecutar (línea 5); por ejemplo, si
se elige el valor 2, se ejecuta \texttt{Verify.getInt(0, scope)} para obtener el valor entero en el rango [0, scope] para usar como argumento del método 
\texttt{list.addLast(value)} (línea 15 y 16). 
\pp{Hay que ponerle nombres a
las variables del los getInt, sino es difícil explicar para qué sirven. 
Además, en la línea 5 es getInt, no random}. \cacho{cambie lo de random por getint y explique mejor}
\pp{Continuar conel ejemplo..}

\cacho{
En la siguiente iteración del bucle (línea 4), JPF volverá a elegir no
determinísticamente un nuevo método entre los disponibles (por ejemplo,
\texttt{removeFirst()} o \texttt{offer(value)}), construyendo así distintas
secuencias de llamadas a la API.
Cada combinación posible de decisiones en \texttt{Verify.getInt} (tanto el
método elegido como los valores generados) corresponde a una exploración distinta
del espacio de búsqueda. \emph{JPF} las explora de forma exhaustiva hasta alcanzar todas
las configuraciones de listas posibles dentro del límite de tamaño
(\emph{scope}).
...
de la API de \texttt{LinkedList}},label={lst:driverAPI},captionpos=b]
...
\pp{Por qué no poner más métodos para que ocupe más espacio la figura? 
Este ejemplo lo usé para la sección de drivers en builders}
\cacho{Cambie a lista y agregue mas metodos}
...
Capitulo \ref{cap:builders}). Como se muestra en la Sección \ref{sec:experiments-jpf-driver} (experimentos
de drivers), estas técnicas pueden ayudar al usuario a la 
definición de drivers más eficientes. 

%Esta técnica actúa como paso previo a la construcción del controlador
