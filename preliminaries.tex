...
\section{Testing}
\cacho{Todo esta seccion de testing es nuea, le di otra vuelta agregando algo de exhaustivo.}
El \emph{testing} es una de las estrategias más directas y extendidas para evaluar el
comportamiento de un programa. En términos generales, consiste en ejecutar el
software bajo condiciones controladas y contrastar los resultados observados con
los resultados esperados (el \emph{oráculo}). Dado que el espacio de entradas de un
programa suele ser inmenso o incluso infinito (por ejemplo, cuando procesa
enteros no acotados), en la práctica se selecciona un subconjunto representativo
de casos de prueba, conocido como \emph{suite de tests}.

El propósito de una \emph{suite} es proporcionar evidencia suficiente sobre la
corrección funcional del sistema, incluso para situaciones no ejercitadas
explícitamente. En procesos de desarrollo contemporáneos, el testing dejó de ser
una actividad exclusivamente posterior a la implementación: en enfoques ágiles
se integra de manera continua, acompañando la evolución del diseño e
influenciando decisiones de implementación.

El \emph{testing exhaustivo acotado} propone explorar \emph{todas} las entradas
válidas dentro de un dominio finito y previamente delimitado. La acotación
transforma un problema inabordable (exhaustivo sobre un dominio infinito) en
uno factible, aunque potencialmente costoso, al restringir el tamaño y la forma
de las entradas.

Dado un programa bajo análisis y una especificación de límites (\emph{scope}) para
sus entradas, el \emph{testing exhaustivo acotado} consiste en: (i) generar \emph{todas} las entradas válidas que
respeten dicho \emph{scope}; y (ii) ejecutar el programa con cada entrada,
verificando que la salida cumpla el oráculo correspondiente.

El interés del \emph{testing exhaustivo acotado} se incrementa cuando las entradas son estructuras con
restricciones (por ejemplo, grafos, árboles o colecciones con invariantes). En estos casos, además de acotar tamaños
(número de nodos, profundidad, etc.), es necesario garantizar que sólo se
generen estructuras válidas de acuerdo con las restricciones declaradas.

La ejecución manual de \emph{testing exhaustivo acotado} es impracticable salvo para dominios triviales, por
lo que la automatización es esencial. Típicamente, el \emph{pipeline} involucra:
\begin{enumerate}
  \item \textbf{Generación} sistemática de entradas dentro del \emph{scope} acotado,
        filtrando únicamente las válidas.
  \item \textbf{Ejecución} del programa con cada entrada.
  \item \textbf{Chequeo de oráculo}, comparación con el resultado esperado o con
        propiedades que deben verificarse.
\end{enumerate}

Para la generación en dominios acotados con restricciones, se emplean técnicas
de \emph{constraint solving} y búsqueda, tales como retroceso (\emph{backtracking}),
model checking, SAT/SMT, o enfoques híbridos que combinan podas estructurales
con enumeración guiada. Estas técnicas permiten reducir el espacio de búsqueda
evitando entradas inválidas o isomórficas, y hacen viable la exploración
exhaustiva dentro del \emph{scope} definido.

\cacho{Saque la seccion de korat, si hace falta la agrego pero ms completa.}
...
\cacho{Mejorar y ampliar, buscar PICT de Pablo}
El emph{testing}  es una técnica fundamental para mejorar la confiabilidad del software
...
tests \cite{Boyapati02,Khurshid01,Fraser11,Visser05,Pasareanu:2010,Cadar08,Tillmann05,Ma15,Ponzio:2016,Rosner14}. 
Uno de los enfoques más importantes para esta tarea es la generación aleatoria
de tests \cite{Chen10,Li18,Ramler12,Shamshiri15}.
...
fallas en distintos tipos de software muy utilizados \cite{Pacheco07,Pacheco08,just2014mutants}
...
\cacho{Ver esto, que tipo de ejemplo poner.}
