...
\emph{debugging} \pp{Acá hay que citar papers de JPF.} \footnote{\url{https://github.com/javapathfinder/jpf-core}}
...
Para analizar el algortimo \ref{alg:propiedad-a-chequear}, JPF necesita mecanismos para generar
...
código.}. Así, las entradas generadas por el
...
la propiedad de listas, no me sirve el driver de árboles para este ejemplo}. 
...
métodos de la API (línea 4 \pp{la inicialización está mal, debería ser $0 <= i <=
maxLength$}). A cada operación se le asigna un
...
se elige el valor 2, se ejecuta \texttt{clear()} \pp{Hay que ponerle nombres a
las variables del los getInt, sino es difícil explicar para qué sirven. 
Además, en la línea 5 es getInt, no random}.
...
(enteros en el ejemplo), y se ejecuta la misma. Por ejemplo, \pp{Continuar con
el ejemplo..}
