...
En adelante, se presenta la técnica desarrollada llamada BEAPI, que tiene como objetivo mejorar la generación exhaustiva acotada (BEG, por sus siglas en inglés, \emph{Bounded Exhaustive Generation}). \cacho{Agregar en preliminar? o referencia a book?}
...
\cacho{No se si es necesario, lo tenia hecho de antes}
...
\cacho{Lo explique antes, pero creo que hace falta como una introducción a lo que es el repOK}
...
% este no sigue extiendo el arbol de búsqueda. En la figura el scope especificado es 3 (vale aclarar, que el nodo ficticio cuanta como nodo de la estructura.)
...
A continuación se muestra un pseudocódigo de \emph{BEAPI} en la Figura \ref{alg:beapi}. 
      ...
      \While{\textbf{true}}{
          ...
          \For{$m(T_1,\ldots,T_n):T_r$ en métodos}{
            ...
            \For{$(s_1,\ldots,s_n) \in seqs_{T_1} \times \ldots \times seqs_{T_n}$}{
              ...
              $(o_1,\ldots,o_n,o_r, fallo, excepción) \leftarrow$ ejecutar($nuevaSec$)\;                
              \If{fallo}{
              ...
              \For{\textbf{cada} tipo $T_i$ en $\{T_1,\ldots,T_n,T_r\}$}{
                  \If{$T_i$ es de referencia \textbf{y} $c_i \notin canónicos$}{
                      $canónicos \leftarrow canónicos \cup \{c_i\}$\;
...
\cacho{Hace falta poner como se corre, no lo veo, es de tool paper eso}
...
Esta tarea es llevada a cabo por el módulo \emph{generador de secuencia de test}, mostrado en la Figura~\ref{fig:beapi-architecture}.
...
El módulo \emph{clasificador}, también ilustrado en la Figura~\ref{fig:beapi-architecture}, 
