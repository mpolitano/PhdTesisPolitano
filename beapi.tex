...
(como \emph{Korat}\cite{Boyapati02}). Finalmente, en la Sección \ref{sec:beapiTechnique} se presenta un pseudocódigo del algoritmo de BEAPI.


\section[Motivación]{Motivación}\label{sec:motivating-example}
...
\end{lstlisting}
\caption{\texttt{repOK} de \texttt{NodeCachingLinkedList} tomado del benchmark de \texttt{ROOPS}}
\label{fig:NCL-repOK}
\end{figure}
...
utilizado para evaluar herramientas automáticas de testing/análisis de software \cite{Pasareanu:2010,Rosner15,Galeotti:2010,galeotti2020,Boyapati02,Khurshid01}.
...
\begin{itemize}
    ...
    \item una suite de tests \textsf{JUnit} \cite{junit} con las secuencias de test 
...
(\emph{feedback-directed test generation} \cite{Pacheco07,Pacheco08}, ver Sección \ref{sec:feedback-directed-test-gen}). 
Al igual que otros enfoques de generación de tests basados en la API, 
\textsf{BEAPI} construye secuencias de test \cite{Pacheco07,Pacheco08} (secuencias de invocaciones a
...
Si no hay fallas, BEAPI genera una suite de tests en formato \textsf{JUnit}. 
...
objetos creados por las mismas por exceder los scopes. Como ejemplo, la Figura \ref{sec:scope} 
muestra una secuencia de test generada usando el método constructor
...
Nuestra técnica, basada en generación de tests guiada por feedback, necesita un
...
``str3''. Además, se pueden proveer valores para todos los tipos incluyendo floats, doubles, etc. 
...
linealización de objetos (ver más adelante en la sección \ref{sec:stateMatching}), que se lleva a cabo luego de la ejecución de las
...
\end{lstlisting}
\caption{Programación defensiva en un método en \texttt{NodeCachingLinkedList}.}
...
Cada nodo del árbol representa una secuencia test generada por \textsf{BEAPI}
...
\emph{linealización} \cite{Iosif02,Xie04}, que convierte un heap con raíz en
...
cuando los objetos exceden el \emph{scope}, e ignorar ciertos campos de los objetos (para la versión original, véase \cite{Xie04}). 
El procedimiento \texttt{linearize} inicia un recorrido en
...
\emph{escalada de colinas} (ver Sección \ref{alg:approachHC}), que logra un mejor rendimiento en tiempo de
ejecución. 
...
computar los métodos generadores de objetos con precisión (ver Sección \ref{sec:experimentalIdentificacion}).
...
produzca objetos más grandes, y con ellos es posible ejercitar más exhaustivamente 
...
\end{lstlisting}
\caption{Pseudocódigo de \textsf{BEAPI}}
...
Si se lanza una excepción que corresponde a un mal uso de la API (ver Sección \ref{sec:defensiveProgramming}), \texttt{BEAPI} descarta la secuencia de test
...
Esto garantiza que \textsf{BEAPI} nunca crea objetos más grandes que lo permitido por el alcance.
