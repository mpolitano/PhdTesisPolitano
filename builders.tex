...
\cacho{Lo siguiente de las figuras fue agregado, no lo puedo pintar en rojo. CACHO ELIMINAR ESTE COMENTARIO LUEGO}
...
En otras palabras, modificamos el algoritmo de Randoop presentado en las seccion  \ref{sec:feedback-directed-test-gen} para generar secuencias de llamadas a métodos con la siguiente estructura: 
}
\vspace{10pt} 

$metodoGenerador^* + metodoNoGenerador$. 

\cacho{TENGO QUE ENTENDER COMO REESCRIBIR ESTE ALGORITMO}
El algoritmo 
...
Es decir, primero generamos secuencias compuestas exclusivamente por métodos generadores y luego, una vez que alcanzamos el tamaño de las secuencias deseadas (\emph{scope}), 
añadimos un método no generador de objecto seleccionado al azar.  

Este método no generador es necesario para garantizar la cobertura de líneas y ramas de código en aquellos métodos que no son generadores pero que, no obstante, 
resultan clave para evaluar la calidad general de las secuencias generadas y su capacidad para ejercitar la API de manera efectiva.  

La motivación detrás de esta modificación radica en que los subconjuntos de métodos que obtienen una mayor cobertura de la API 
nos proporcionarán indicios valiosos sobre la calidad de dicho subconjunto. 
Por lo tanto, nuestra función de valoración tiene como objetivo maximizar la cobertura de ramas de la API bajo prueba, 
al como se describe en la sección de preliminares \ref{sec:coverage}.
...
Por ejemplo, la posición 0 puede corresponder a $Método_1$, la posición 1 a $Método_2$, y así sucesivamente.  
...
\cacho{Poner la flecha al medio}
...
\cacho{Para nuestro caso concreto, como ya explicamos, cada posición del cromosoma representa un método específico de la API, entonces
una mutación en la posición 2 implicaría activar o desactivar la ejecución del método $Método3$.
...
\begin{algorithm}
  ...
  \FOR{$i \gets 0$ \TO $numEvoluciones$}
      \STATE $P \gets$ seleccionar $popSize$ cromosomas con mejor aptitud
      \FOR{$j \gets 1$ \TO $\text{tasaC} \times |P|$}
          \STATE $C_1, C_2 \gets$ seleccionar dos cromosomas con mejor aptitud
    ...
    \IF{la aptitud de $C$ es constante durante $maxIterConst$}
...
\cacho{Ver esta parte anterior} (línea 18 a 29). Ver figura \ref{fig:succ-hillClimbing}
