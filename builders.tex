...
\begin{algorithm}[htbp]
    ...
    \Fn{\successor{$state$, $const$}}{
        ...
        \For{$i \gets 0$ \KwTo \#M}{
            \If{$s[i] == 0$}{
                \pp{qué es builders? No se entiende la condición}\; \cacho{Esto ya lo charlamos en una meet, lo hicimos asi para detectar constructores primero y usar solo eso}
...
Además, la búsqueda se guía mediante una de las funciones objetivo definidas en la Sección \ref{sec:fitness}.
...
la búsqueda anterior (línea 1 a 5).
...
(indicando que el método correspondiente no está seleccionado) se evalúa si puede activarse en función de si se está buscando métodos constructores de la API o no. 
