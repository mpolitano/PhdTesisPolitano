...
\begin{itemize}
    ...
    \item \textbf{Métodos observadores:}  
        Estos métodos permiten inspeccionar el estado de la lista sin
        modificarla. Por ejemplo:
    \begin{itemize}
        \item \texttt{contains(Object)}: Verifica si un elemento está presente o
            no.
...
\noindent
A partir de esta API, los objetos de \texttt{NodeCachingLinkedList} pueden
...
\vspace{5pt} 
    ...
    (0)  NodeCachingLinkedList<Integer> l = new NodeCachingLinkedList<>();
    (7)  l.addFirst(3);
    (7)  l.addFirst(-1);
    \end{lstlisting}

\item Figura (b):
    Esta instancia se logra agregando dos nodos a la lista principal y luego
    eliminando uno de ellos. En este caso el primer nodo agregado a la lista principal
    se mueve a la lista caché. El objeto se construye ejecutando la secuencia de
    test:
    \vspace{5pt} 
...
\item Figura (c): 
    ...
    para pasarlos a la lista caché, esto es, con el siguiente test: 
    \vspace{5pt} 
...
Notar que varios métodos en la Tabla \ref{tab:ncl-api} están clasificados como
observadores (columna Observador?), lo que significa que no modifican los objetos sobre los que operan, ni son útiles para crear nuevos objetos. 
Los métodos observadores siempre son superfluos y nunca deben incluirse en un conjunto de generadores de objetos minimal. Por esta razón, nuestros enfoques de identificación de métodos generadores intentan reconocerlos de antemano y descartarlos.
...
El primer algoritmo que presentamos es un enfoque basado en algoritmos
genéticos, que utiliza principios inspirados en la evolución biológica
\cite{Goldberg:1989}.
...
El segundo algoritmo que proponemos es un enfoque \emph{greedy}, específicamente
una variante de un algoritmo de ascenso de colinas (\emph{hill climbing}) \cite{Russell:2009,Cormen2009}. Este enfoque se centra en mejorar iterativamente un conjunto inicial de métodos generadores mediante la adición de métodos.

Cada uno de estos algoritmos ofrece ventajas y desventajas en términos de
rendimiento y precisión. Más adelante comparamos y evaluamos estos algoritmos en
diferentes casos de estudio para determinar sus características en la tarea de 
identificar conjuntos suficientes y minimal de generadores de objetos.
...
Antes de ejecutar cualquiera de nuestros algoritmos, resulta fundamental
identificar este tipo de métodos, ya que no contribuyen a la construcción de
objetos.
...
Para automatizar la identificación de estos métodos, utilizamos la herramienta de análisis estático \emph{Infer} \footnote{\url{https://fbinfer.com/}}, desarrollada por Facebook. 
\textsf{Infer} ofrece funcionalidades avanzadas para analizar código fuente y, en
particular, permite identificar automáticamente los métodos observadores (también
denominados \emph{métodos puros} en la bibliografía \cite{Salcianu:2005,Huang:2012}).
...
Si bien \textsf{Infer} puede generar falsos negativos —no clasificar como
observador a un método que en realidad sí lo es—, su comportamiento es
conservador y típicamente evita falsos positivos. 
Es decir, rara vez clasificará como observador a un método que en realidad
produce modificaciones en los objetos sobre los que opera. 
...
Los métodos reportados como observadores por \textsf{Infer} son descartados antes de comenzar la búsqueda de métodos generadores de objetos, y no se consideran en ninguna fase posterior del proceso.
En síntesis, \textsf{Infer} actúa como una etapa de preprocesamiento que combina eficiencia y utilidad práctica: permite reducir el número de métodos a analizar, sin necesidad de intervención manual, manteniendo un nivel de precisión suficiente para nuestros casos de estudio.
...
\caption{API de NodeCachingLinkedList de Apache luego de descartar los métodos
observadores con Infer.}
...
\begin{figure}[H]
  ...
  \caption{Ejemplo de estado de nuestros algoritmos.}
  \label{fig:cromosoma}
\end{figure}

En este caso, las posiciones 0, 7 y 16 del vector están establecidas como verdaderas (según el orden asignado a los métodos de la API), mientras que las demás posiciones están establecidas como falsas. Esta configuración representa el subconjunto de métodos:
\vspace{5pt} 

\begin{lstlisting}[numbers=none, caption=Métodos generadores de objetos
representados por el cromosoma de la Figura \ref{fig:cromosoma}., captionpos=b, frame=tb , xleftmargin=0pt, basicstyle=\scriptsize]
  (0)  NodeCachingLinkedList()
  (7)  addFirst(Object)
  (16) removeFirst()
\end{lstlisting}
...
Así, la primera función objetivo propuesta evalúa la capacidad de generación de objetos de un candidato $C$ realizando una generación exhaustiva acotada \cite{Politano23} usando los métodos en $M$. La función objetivo asigna un puntaje a $C$ basándose en la cantidad de objetos únicos producidos durante la generación exhaustiva acotada. Esto refleja la diversidad de objetos que se pueden generar utilizando los métodos dados. Los subconjuntos de métodos que generan más objetos serán los candidatos preferidos para ser elegidos por nuestro algoritmo como generadores de objetos.
Nuestro enfoque utiliza un generador exhaustivo acotado eficiente basado en métodos de la API \cite{Politano23}. Este generador se denomina BEAPI y es una de las contribuciones principales de este trabajo (ver Capítulo \ref{cap:beapi}).
...
\begin{lstlisting}[label=fig:rankParameters,caption=Pesos de los distintos tipos de parametros, captionpos=b,frame=tb, float=t]
Boolean=1
Integer,Char=2
...
\begin{lstlisting}[numbers=none, caption=Conjunto de métodos candidato \( M_1 \)]
  NodeCachingLinkedList()
  addFirst(Object)
  removeFirst()
\end{lstlisting}

\begin{lstlisting}[numbers=none, caption=Conjunto de métodos candidato \( M_2 \)]
  NodeCachingLinkedList()
  addFirst(Object)
\end{lstlisting}
...
Suponiendo un \emph{scope} de 3 para la generación exhaustiva acotada, con \(
M_1 \) podemos generar 18 objetos, mientras que con \( M_2 \) generamos 12. 
%Las funciones objetivo son entonces:
%
%\[
...
Esto implica que \( M_1 \) es un candidato más adecuado para métodos generadores
de objetos, ya que permite construir un mayor número de instancias. 
En particular, \( M_2 \) no puede generar aquellas instancias que requieren
eliminar elementos de la lista y mover nodos a la caché, lo que sí se logra con 
\( M_1 \), usando el método \texttt{removeFirst()}.

Veamos otro ejemplo, en donde ambos conjuntos generan la misma cantidad de objetos (18, para scope 3), 
pero  \( M_3 \) es minimal y \( M_4 \) no lo es:

\vspace{5pt}

\begin{lstlisting}[numbers=none,label=fig:NCLbuilders3, caption=Conjunto de
métodos candidato \( M_3 \)]
  NodeCachingLinkedList()
  addFirst(Object)
  removeFirst()
\end{lstlisting}

\begin{lstlisting}[numbers=none,label=fig:NCLbuilders4, caption=Conjunto de
métodos candidato \( M_4 \)]
  NodeCachingLinkedList()
  addFirst(Object)
  add(Object)
  removeFirst()
...
%Ambos conjuntos logran generar la misma cantidad de objetos. 
Sin embargo, \( M_4 \) utiliza un método más, por lo que tiene una redundancia
mayor (notar que 20 es el número de métodos en la API):
\[
R(M_3) = 20 - 3 = 17 \text{, } R(M_4) = 20 - 4 = 16
\]
Dado que nuestra función objetivo favorece subconjuntos más reducidos caso de
empate en la cantidad de objetos generados, se concluye que:
...
Como la generación exhaustiva acotada puede ser costosa computacionalmente,
proponemos aquí una heurística para computar la función objetivo que podría 
dar lugar a una mayor eficiencia en los algoritmos de cómputo de métodos
generadores. Así, la segunda función objetivo se basa en generar test suites
de manera automática (aleatoria) utilizando un subconjunto candidato de métodos,  
...
programa que son ejercitadas por los tests (ver Sección \ref{sec:coverage}).
Esta métrica estima en qué medida los tests generados a partir del subconjunto de métodos 
cubren las distintas partes del programa, 
que suele ser un aspecto clave de la calidad de las suites de test obtenidas.
...
En el contexto de este trabajo, hemos realizado modificaciones en la
herramienta \textsf{Randoop} (descrita en la sección \ref{sec:feedback-directed-test-gen})
de manera que las test suites generadas sean de utilidad para averiguar si un conjunto de 
métodos dados es un conjunto de métodos generadores.

La heurística para averiguar si un
subconjunto de métodos $M$ es un conjunto de generadores se basa en
realizar una primera ejecución de Randoop utilizando sólo los métodos en $M$ para
generar un conjunto de secuencias de test para ``producir objetos'' $tests_{obj}$. En
...
Luego, estas secuencias ``creadoras de objetos'' se extienden en una ejecución de 
Randoop subsiguiente,
pero esta vez usando los métodos restantes de la API. El objetivo de esta
segunda ejecución es maximizar la cobertura de código. 
...
Por ejemplo, si la primera ejecución no puede construir buenos objetos, extender 
...
generar secuencias de test, $tests_{obj}$ (línea 6), que usan estos métodos para construir objetos. 
Esta ejecución utiliza un 40\% del tiempo total
...
la estrategia de generación aleatoria descrita en la Sección
\ref{sec:feedback-directed-test-gen}, basada en el enfoque de Randoop. 
...
Primero, se genera una test suite para un conjunto de métodos dado $M$, con un tiempo límite
de generación $S$ fijo (determinado experimentalmente). Luego, se mide  
 ...
 $Cob$ aplicando la fórmula \ref{eq:cobertura} con estos valores.

En la práctica, dada la aleatoriedad de \textsf{Randoop}, con algunas semillas
la herramienta logra mayor cobertura que con otras. Por lo tanto, para lidiar
con la aleatoriedad y computar $Cob$ con mayor precisión, lanzamos $k$ hilos
(\emph{threads} ejecutando el algoritmo \ref{alg:fitnessRandoop} en paralelo 
con $k$ semillas aleatorias distintas. Luego, el resultado retornado para la 
función objetivo es el promedio de los 
resultados de $Cob$ obtenidos en los distintos hilos. 
...
\begin{itemize}
    \item $Cob(M)$ (Cobertura): Es el promedio del número de ramas y lineas cubiertas
        (\ref{sec:coverage}) por las
        suites generadas por los $k$ hilos del algoritmo \ref{alg:fitnessRandoop}, utilizando $M$ como
        entrada. 
...
\item $R(M)$ (Redundancia): Diferencia entre el número total de métodos de la
    ...
    función objetivo basada en generación exhaustiva acotada de la sección \ref{sec:fitnessGE}.
...
Volvamos al ejemplo que usamos anteriormente en este capítulo. Consideremos los siguientes conjuntos de métodos:
...
%\]
Al ejecutar el algoritmo \ref{alg:fitnessRandoop} con cada conjunto como
entrada, se generan suites de tests que luego son ejecutadas para analizar su
cobertura.
En este caso, asumiendo un solo hilo para simplificar ($k=1$), la cobertura \(Cob\), se computa a partir de la proporción de líneas 
cubiertas respecto del total de líneas (232) y de la proporción de ramas cubiertas respecto del total de ramas (132),
asignando un peso de 0.4 a las líneas y de 0.6 a las ramas.
Con \( M_1 \) se cubrieron 192 líneas y 101 ramas (\(Cob(M_1) = 0.77\)), mientras
que con \( M_2 \) se cubrieron 175 líneas y 97 ramas (\(Cob(M_2) = 0.74\)).

%Para la redundancia de métodos, tal cual la sección anterior, se define como \( R(M) = M_t - |M| \), donde \( M_t \) es el número total de métodos en la API (en este caso, 20. Ver \ref{tab:ncl-api-infer}). 
...
%
El conjunto \( M_1 \) logra una mejor cobertura y por lo tanto:

\[
f(M_1) > f(M_2)
\]

Esto se debe a que la inclusión del método \texttt{removeFirst()} en \( M_1 \)
permite generar objetos más variados (con cachés no vacías), que sirven para ejercitar 
comportamientos de la API que no pueden ejercitarse con los objetos generados
con \( M_2 \). En particular, \( M_2 \) no permite generar objetos con listas caché no vacías,
lo que impide cubrir ciertas ramas del código.
...
En esta sección, presentamos dos algoritmos para identificar
métodos generadores de objetos a partir una API. 
...
(por ejemplo, la aptitud de la población puede tomarse como la aptitud de su
individuo ``más apto'').
...
A continuación explicaremos en detalle los distintos componentes del algoritmo genético que hemos implementado.
...
Cómo es típico para los hiper parámetros de los algoritmos genéticos determinamos el tamaño de la población empíricamente, probando distintos valores
eligiendo aquel que mejor rendimiento obtuvo en experimentos preliminares.
...
En nuestro enfoque, hemos utilizado un operador de selección tipo torneo (\emph{tournament}) \cite{Goldberg:1989}. Este enfoque funciona de la siguiente manera: se selecciona aleatoriamente un grupo de $t$ individuos de la población y se los pone a ``competir'' entre sí. El individuo más apto (con mejor valor de la función objetivo) dentro de este grupo es seleccionado para participar en la reproducción. El tamaño del torneo $t$ (la cantidad de individuos que compiten entre sí) es un hiper parámetro del algoritmo. En nuestro caso, utilizamos un tamaño de torneo de 4, seleccionado empíricamente.

El operador de selección tipo torneo ofrece una forma eficiente de elegir individuos y típicamente da lugar una buena diversidad de individuos en la población \cite{Goldberg:1989}. Es eficiente porque los torneos pueden paralelizarse, debido a que son independientes entre si. Además, es un operador muy flexible, ya que permite adaptar la velocidad de convergencia del algoritmo variando $t$. Valores más grandes de $t$ producen una mayor probabilidad de selección de los individuos más aptos, lo que acelera la convergencia, y viceversa. 

Nuevamente, elegimos este operador de selección porque dio los mejores
resultados en experimentos preliminares (también probamos otros operadores típicos como selección por ruleta, selección por ranking \cite{Goldberg:1989}.
...
Este valor fue determinado experimentalmente para equilibrar la diversidad de la exploración y la preservación 
...
correspondiente en el individuo, de acuerdo a la tabla \ref{tab:ncl-api-infer}, seria el método \texttt{add(Object)}.
...
\begin{algorithm}
  ...
  \label{alg:enfoqueGA}
  \begin{algorithmic}[1]
  \STATE $pop \gets$ población inicial de cromosomas con un solo gen en 1
...
La población inicial se construye en la línea 1, generando los cromosomas de
manera que cada uno representa un subconjunto de métodos con exactamente un método activo.
Esta decisión inicial permite explorar combinaciones desde los métodos más simples hacia conjuntos más complejos 
a lo largo de la evolución.
En la línea 2 y 3 se define la proporción de la población que se va a tomar como
descendencia ($offspringSize$), la misma es definida como un 60\% del tamaño total de la población $N$. 
El 40\% restante se mantiene como sobrevivientes de la población actual
($survivorSize$). 
A partir de allí, el algoritmo entra en el ciclo de evolución de la población, 
que se repite por un número máximo predeterminado de generaciones $numEvo$ (líneas 4-28).

En cada iteración, se seleccionan los individuos sobrevivientes ($survivors$) mediante un
proceso de \textit{selección por torneo} (línea 5), utilizando el parámetro 
$tournamentSize$ como el tamaño de cada torneo. 

A continuación, se genera una descendencia en la línea 6 en un ciclo de 
reproducción que genera $offspringSize$ descendientes.
Cada nuevo individuo se genera a partir de dos padres seleccionados también por torneo (líneas 8-9). 
Con una probabilidad $crossoverRate$ se aplica el operador de
\textit{cruzamiento de dos puntos} para generar el descendiente (línea 11); en caso contrario, se elige el más apto entre los dos padres (línea 13).
Luego de generar los descendientes mediante cruzamiento ($offspring$), se aplica
el operador de \textit{mutación} (líneas 17-22) sobre cada uno de ellos. 
La mutación se aplica a cada gen de cada cromosoma, con probabilidad
$mutationRate$, y si el cromosoma mutado es diferente del original, se reemplaza
en $offspring$.

La nueva población para la siguiente generación se construye tomando la unión de
los sobrevivientes y la descendencia (línea 23). A partir de esta población, se
determina el mejor cromosoma actual $C$ (línea 24).
El algoritmo verifica si la aptitud del mejor cromosoma se mantiene constante 
durante un número de generaciones consecutivas igual a $maxIter$ (líneas 25 a 27). 
Si esto ocurre, el ciclo principal se detiene anticipadamente y el algoritmo
termina.

Finalmente, el mejor cromosoma encontrado se retorna como resultado del algoritmo (línea 29).

Notar que tanto la función basada en generación exhaustiva acotada presentada en
la Sección \ref{sec:fitnessGE}, como la función basada en cobertura de código
introducida en la Sección \ref{sec:fitnessRandoop}, pueden utilizarse para
evaluar la aptitud de los individuos en el algoritmo genético.  
...
Los valores utilizados en nuestras evaluaciones son: tamaño de población $N =
50$, tasa de cruzamiento $crossoverRate = 0.50$, tasa de mutación $mutationRate
= 0.05$, número máximo de generaciones $numEvo = 20$, máximo número de 
iteraciones sin mejoras en la aptitud del mejor cromosoma $maxIter = 5$.
Existe un compromiso al elegir un valor para $numEvo$: un número mayor tiene el
potencial de mejorar la precisión del algoritmo pero incrementa su tiempo de ejecución, 
mientras que un número menor lo hace más rápido pero podría no encontrar el
mejor conjunto de generadores de objetos. Similarmente, un valor bajo para
$numEvo$ hace que el algoritmo converja rápidamente, posiblemente a expensas de
la precisión del resultado, y viceversa.
El valor del tamaño del torneo para la selección también se eligió experimentalmente
($tournamentSize=4$).

Para implementar el algoritmo genético, utilizamos una biblioteca popular
escrita en Java, llamada \textsf{Jenetics}\footnote{\url{https://jenetics.io/}}
(versión 5.2.0). Esta biblioteca está diseñada específicamente para algoritmos evolutivos 
y nos proporcionó las herramientas necesarias para desarrollar nuestro enfoque genético.
\textsf{Jenetics} es una biblioteca robusta y versátil que ofrece una amplia gama
de funcionalidades para la implementación de algoritmos genéticos. Además,
\textsf{Jenetics} es muy fácil de usar, con una documentación completa y una
comunidad activa de usuarios que proporciona soporte y ayuda. Una característica
relevante de \textsf{Jenetics} es que permite realizar la evaluación de la
aptitud de los individuos (así como otras operaciones de los algoritmos genéticos) 
de la población en paralelo, 
%utilizando el \emph{ForkJoinPool.commonPool()}, 
lo que mejora notablemente el rendimiento en sistemas con múltiples núcleos. 
Esta paralelización se gestiona de forma transparente, sin necesidad de implementar 
hilos manualmente, y puede personalizarse en caso de requerirse un control más detallado 
del entorno de ejecución.

%Nos permitió definir y manipular genes, cromosomas y poblaciones, así como utilizar operadores genéticos como selección, cruzamiento y mutación. Además, cuenta con un sólido conjunto de herramientas de optimización y técnicas de evolución que nos permitieron adaptar el algoritmo a nuestras necesidades específicas.
...
\subsection{Algoritmo de Escalada de Colinas}
\label{alg:approachHC}

El segundo algoritmo, que denominamos \textit{hill climbing}, es un algoritmo de búsqueda local 
basado en la escalada de colinas \cite{Russell:2009, Cormen2009, kleinberg2006}.
Es un algoritmo greedy que selecciona en cada paso el estado vecino con mayor
valor en la función objetivo. 
Así, el algoritmo de escalada de colinas parte de un estado inicial y lo mejora sucesivamente hasta alcanzar un estado que no puede ser mejorado. 
...
\begin{algorithm}[htbp]
    ...
    \Fn{\mejorSuccesor{$successors$}}{
        $curr \gets -INF$\;
        \For{$i \gets 0$ \KwTo \#successors-1}{
            \If{$s[i] > bestVal$}{
                $bestVal \gets f(s[i])$\;
                $best \gets s[i]$\;
            }
...
El algoritmo \ref{alg:enfoqueHC} muestra el pseudocódigo del enfoque propuesto.
La representación que hemos utilizado del problema es la explicada en la Sección \ref{sec:estados}, donde los estados se modelan como vectores de bits.
La búsqueda se guía mediante una de las funciones objetivo definidas
anteriormente: la función basada en generación exhaustiva acotada (Sección
\ref{sec:fitnessGE}), o la función basada en cobertura de código (Sección \ref{sec:fitnessRandoop}).

El proceso comienza con la función principal \texttt{computeBuilders}, que implementa la búsqueda.
La inicialización consiste en definir un estado inicial representado por un vector de ceros, 
que indica que ningún método está seleccionado. Este estado se pasa como
parámetro a la función $hillClimbing$, junto con un valor lógico $const=true$ que indica que
se debe utilizar $hillClimbing$ para seleccionar sólo métodos constructores
(línea 3). 
Una vez seleccionados los constructores ($cons$), el algoritmo invoca nuevamente
a $hillClimbing$ (línea 4) para realizar la búsqueda de los métodos modificadores que
constituyen un conjunto de generadores de objetos (a partir de los mejores constructores seleccionados anteriormente en
$cons$).

La función $hillClimbing$ constituye el núcleo del algoritmo de Hill Climbing, 
y se corresponde con un algoritmo típico de escalada de colina.
Su ejecución parte del estado inicial recibido por parámetro. 
El algoritmo comienza un procedimiento iterativo que busca mejorar el estado
actual (líneas 7-16). Para esto genera todos los posibles sucesores del estado
actual (línea 8) utilizando la función $generarSucesores$ (definida en las líneas 17-28). 
Notar que $generarSucesores$ sólo produce sucesores con métodos constructores si
$const=true$, y crea sucesores con métodos modificadores (no constructores) si
$const=false$.  
Entre los sucesores producidos se selecciona el que presente el mejor valor
de la función objetivo $f$ (línea 9). Si dicho sucesor no mejora el valor del estado actual,
el algoritmo termina devolviendo el estado alcanzado (líneas 10-12). 
En caso contrario, el mejor sucesor pasa a ser el nuevo estado actual (líneas
13-15), y el ciclo continúa.

La función $generarSucesores$ es la
encargada de crear los sucesores. Esta función evalúa cada posición del vector de estado. Si una posición tiene el valor 0 
(indicando que el método correspondiente no está seleccionado) se evalúa si puede activarse en función de si se está buscando métodos constructores o no. 
En caso de cumplir con esta condición, se genera un clon del estado actual en el
que se activa dicha posición, y este nuevo estado se añade a la lista de
sucesores. (línea 18 a 29). Un ejemplo de los sucesores que se pueden generar
para un estado se muestra en la Figura \ref{fig:succ-hillClimbing} (asumiendo que 
sólo el primero es un constructor y se están generando sucesores para métodos
que no son constructores).

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{images/succesores.png}
    \caption{Sucesores de un estado usando $generarSucesores$.}
    \label{fig:succ-hillClimbing}
  \end{figure}
  
La función $mejorSuccesor$ se encarga de seleccionar el mejor
sucesor de la lista proporcionada (líneas 29-37).
Para ello, recorre la lista evaluando el valor de la función objetivo $f$ en
cada sucesor y selecciona aquel que tiene el mayor valor.



\section{Construcción de \emph{drivers} para model checking usando métodos generadores de objetos}
\label{sec:jpf-builders}

En la Sección \ref{sec:jpf}, mostramos que para la verificación acotada de
propiedades para estructuras dinámicas complejas el usuario debe proveer un
\emph{driver} para genere las estructuras (de manera exhaustiva acotada). 

Por ejemplo, la Figura \ref{lst:driverAPI} muestra un driver para generar todos
los posibles \texttt{TreeMap}s con hasta \texttt{scope} nodos, con elementos
en \texttt{[0, scope]}. Notar que, si no se conocen los métodos
generadores de objetos del módulo, el usuario tiene que escribir el driver
utilizando todos los métodos de la API (como en la Figura \ref{lst:driverAPI}). 

\noindent\begin{minipage}{\linewidth}
\begin{lstlisting}[language=Java,basicstyle=\scriptsize\ttfamily, caption={Driver usando todos los métodos
de la API},label={lst:driverAPI},captionpos=b]
private static TreeMap<Integer, Integer> generateStructure(int scope) {
    ...
    for (int i = 0; i < maxLength; i++) {
        switch (Verify.getInt(n_methods - 1)) {
            ...
            case 4:
                ...
                break;
            case 5:
                t.get(Verify.getInt(0,scope));
                break;
            case 6:
                t.putIfAbsent(Verify.getInt(0,scope), Verify.getInt(0,scope));
                break;
            case 7:
                t.replace(Verify.getInt(0,scope), Verify.getInt(0,scope));
                break;
            case 8:
                t.isEmpty();
                break;
...
En caso contrario, es posible que
algunos métodos importantes se omitan, y de esta manera el driver no construya
ciertos objetos que pueden ser importantes para revelar errores en el software
bajo análisis. Por otra parte, la cantidad de caminos a explorar en el driver 
crece exponencialmente con el número de métodos (la cantidad de opciones en el
\texttt{switch}). De esta manera, incluir métodos innecesarios para crear
objetos impacta negativamente en el tiempo de ejecución de la verificación acotada.
Cabe destacar que la API de \texttt{TreeMap} cuenta con 61 métodos.

En este trabajo, se propone utilizar los métodos generadores detectados
automáticamente por nuestros enfoques para construir drivers. El objetivo es acelerar la 
verificación de propiedades y mejorar su escalabilidad respecto de los scopes en casos 
que ocurren típicamente en la práctica (como el caso de \texttt{TreeMap} presentado aquí). 
Los generadores de objetos para \texttt{TreeMap} cuentan con sólo 3 métodos.
Así, utilizando sólo métodos generadores de objetos computados automáticamente
por nuestros enfoques, se construye el driver de la Figura \ref{lst:driverBLD}. 
Este driver genera exactamente los mismos objetos de \texttt{TreeMap} que el de
la Figura \ref{lst:driverAPI} (porque los métodos generadores son suficientes), 
y por lo tanto JPF explora la propiedad bajo análisis de manera
exhaustiva acotada con ambos drivers. Sin embargo, como
se muestra en la sección experimental (\ref{sec:experiments-jpf-driver}), 
eliminar métodos innecesarios del driver produce ganancias significativas en tiempo y 
escalabilidad respecto del scope, para la verificación de propiedades.

\noindent\begin{minipage}{\linewidth}
\begin{lstlisting}[language=Java,caption={Driver con los métodos generadores de objetos},label={lst:driverBLD},captionpos=b]
    private static TreeMap generateStructure(int scope) {
       int maxLength = Verify.getInt(0, scope);
       TreeMap t = new TreeMap();
       for (int i = 0; i < maxLength; i++) {
          switch (Verify.random(n_methods - 1)) {
             case 0:
                t.put(Verify.getInt(0,scope),Verify.getInt(0,scope));
                break;
             case 1:
...
Cabe mencionar que, para los casos de estudio considerados, los drivers tienen una estructura
similar: un ciclo principal en el que se repite la elección no determinística de un
método del conjunto dado (seleccionando no determinísticamente valores para los parámetros
de tipo primitivo, en caso de que existan). Para realizar los experimentos de la
sección \ref{sec:experiments-jpf-driver}, se implementó una herramienta
prototípica que, dados un scope y un
conjunto de métodos, genera automáticamente los drivers para la creación de
estructuras usando los métodos dados (la API completa, o los métodos generadores
de objetos identificados por los algoritmos propuestos). 
