...
\begin{figure}[H]
  ...
  \caption{Ejemplo de representación de estado con un vector booleano}
  \label{fig:cromosoma}
\end{figure}
...
\section{Construcción de \emph{drivers} para model checking acotado usando métodos generadores de objetos}
...
private static TreeMap<Integer, Integer> generateStructure(int scope) {
    ...
    for (int i = 0; i <= maxLength; i++) {
        switch (Verify.getInt(n_methods)) {
            case 0:
...
\cacho{Hecho}
\pp{Poner más métodos para que ocupe más espacio la figura, y de paso se note
más la diferencia para builders vs. no builders.}
...
en \texttt{[0, scope]} \pp{Hay que ver cómo funciona el getInt y actualizar
esto para que den las cotas}\cacho{Resuelto, es hasta scope}. Notar que, si no se conocen los métodos
...
\pp{Falta un método para que coincida con la sección experimental creo.}
...
la Figura \ref{lst:driverAPI} (porque los métodos generadores son suficientes y
minimales), y por lo tanto JPF explora la propiedad bajo análisis de manera
