...
\begin{itemize}
    ...
    \begin{itemize}
        \item \texttt{add(Object)}: Agrega un elemento al final de la lista
            principal \cacho{y no se modifica la lista cache }\pp{No puede ser,
            la cache estaría al pedo si fuera así porque no se reusan los nodos. Revisar}
        \item \texttt{addFirst(Object)} y \texttt{addLast(Object)}: Insertan elementos al principio o al final de la lista principal \cacho{y no se modifica la lista cache}\pp{No puede ser,
            la cache estaría al pedo si fuera así porque no se reusan los nodos. Revisar}
...
\vspace{5pt} 
    ...
    (7)  addFirst(-1);
    \end{lstlisting}
    \pp{Modificar la figura para que contenga elementos (N0, N1 y L0 están al
            pedo, sino las listas no son concretas y las operaciones con object 
        no son ejecuciones válidas.}

\item Figura (b):
    Esta instancia se logra agregando un nodo \pp{No. hay que agregar dos nodos
    y luego eliminar uno.} a la lista principal y luego eliminarlo para que sea insertado en la cache:
    \vspace{5pt} 
...
\item Figura (c): 
    ...
    para pasarlos a la lista cache \pp{Poner valores concretos}:
    \vspace{5pt} 
...
Luego, usamos esta versión de Randoop para computar $Cov$ de la siguiente manera.
Primero, se genera una test suite usando los métodos dados con un tiempo límite
de generación fijo (determinado experimentalmente). Luego, se mide  
 la cobertura de líneas y de ramas obtenida por la test suite, y se computa $Cov$ con
estos valores.
...
\cacho{Debo entender como modificar el algoritmo basandonos en la estructura de
arriba y que se entienda. Explicar linea por linea.} \pp{???}

\pp{Hace falta el número máximo de tests W en el algoritmo o lo podemos sacar? Me suena raro
pero podría estar comiéndome algún caso.}
...
\begin{algorithm}[H]
    ...
    tiempo total $S$, número máximo de tests $W$}
...
que representa un conjunto de métodos $M$, se define como una tripla, al igual que la función de la sección anterior:
...
\pp{El ejemplo está incompleto??? Hay que arreglar todo lo que había que arreglar
    en la sección previa también (ej. valuaciones para los parámetros.}
