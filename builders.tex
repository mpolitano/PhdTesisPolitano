...
en \texttt{[0, scope-1]} \pp{Hay que ver cómo funciona el getInt y actualizar
esto para que den las cotas}. Notar que, si no se conocen los métodos
...
Cabe destacar que la API de \texttt{TreeMap} cuenta con \pp{XXX (poner cuántos
metodos)} métodos.
...
\begin{lstlisting}[language=Java,caption={Controlador con los métodos generadores de objetos},label={lst:driverBLD},captionpos=b]
    private static TreeMap generateStructure(int scope) {
       int maxLength = Verify.getInt(0, scope);
       TreeMap t = new TreeMap();
