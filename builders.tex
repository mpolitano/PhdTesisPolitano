...
% \ref{tab:ncl-api}.
 \pp{hay métodos mal clasificados por Infer. Como iterator.
...
\cacho{Es nuevo}
En el ejemplo motivador presentado en la Sección~\ref{sec:motivacion} y en la Tabla~\ref{tab:ncl-api}, se puede observar que existen numerosos métodos clasificados como observadores. 
...
programa que son ejercitadas por los tests (\ref{sec:coverage}) \pp{hacer referencia a la sección
...
La \textbf{función objetivo} utilizada durante todo el proceso es la definida en la sección~\ref{sec:fitnessGE}, y tiene en cuenta tres componentes: cantidad de objetos generados, minimalidad (métodos usados) y complejidad de parámetros. 
