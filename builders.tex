...
en la Figura \ref{}. \pp{Poner una referencia al algoritmo}
...
\begin{algorithm}[H]
    ...
    \Fn{\FMain{$API$, $M$, $S$, $W$}}{
        ...
        $tests_{obj}, tests_1 \leftarrow$ \FGen{$M_{gen}$, $S_1$, $W$, $\emptyset$}\;
        $prev_2, tests_2 \leftarrow$ \FGen{$M_{nogen}$, $S_2$, $W$, $tests_{obj}$}\;
...
En nuestro caso, utilizamos una probabilidad de mutación de 0.05 \pp{más
adelante dice otra cosa distinta, 0,08}, es decir, cada bit de un individuo tiene un 5\% de probabilidad de cambiar de estado.  
...
elegimos $numEvo=20$, $popSize=30$ \pp{Esto no es lo que dice en la sección
correspondiente, que menciona 100. Tener en cuenta todas las secciones
globalmente a la hora de redactar!}, $cRate=0.35$, $mRate=0.08$ 
y $maxIter=5$. 
