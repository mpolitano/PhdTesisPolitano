...
En otras palabras, modificamos Randoop para generar secuencias de llamadas a métodos con la siguiente estructura: $builders^* + no\_builders$. Es decir, primero generamos secuencias compuestas exclusivamente por métodos *builders* y luego añadimos un método *no builder* seleccionado al azar.  
...
En el contexto de un algoritmo genético, la población se refiere a un conjunto de individuos o soluciones candidatas \cite{Goldberg:1989}. Cada individuo de la población representa una posible solución al problema que se está tratando de resolver. 
Para nuestro algoritmo representaremos los individuos como vectores de bits, como se describió en la Sección \ref{sec:estados}. 
La población inicial se crea mediante  la generación aleatoria de $p$ individuos (vectores de bits), donde $p$ es un hiperparámetro del algoritmo genético.
