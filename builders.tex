...
cobertura.
En este caso, asumiendo un solo hilo para simplificar ($k=1$), la cobertura \(Cob\), se computa como la suma del número de
líneas y ramas cubiertas \pp{según la fórmula anterior es 0.4 * lineas y 0.6 por
ramas. Corregir los números en este párrafo.}. \cacho{Aca me parece que hay que cambiar la formula, porq dice que se calculan respecto el total de lineas y ramas, y no es asi. Ya cambie los valores}
Con \( M_1 \) se cubrieron 192 líneas y 101 ramas (\(Cob(M_1) = 137,4\)), mientras
que con \( M_2 \) se cubrieron 175 líneas y 97 ramas (\(Cob(M_2) = 128,2\)).
