...
\cacho{1 - Cambiar puros por observadores en todos lados}
...
\cacho{Revisar todo esto, mejorar, agregar dibujos, y explicacion de algoritmo}
...
\begin{algorithm}[H]
    ...
    \Fn{\hc{$initialState, constructores$}}{
        $curr \gets initialState$\;
        \While{true}{
            $successors \gets$ \successor{$curr$, constructores}\;
    ...
    \Fn{\successor{$state$, $constructores$}}{
        $initial \gets []$\;
        \For{$i \gets 0$ \KwTo \#M}{
            \If{$s[i] == 0$}{
...
El algoritmo presentado (en la figura \ref{alg:enfoqueHC}) implementa una versión del enfoque $Hill Climbing (HC)$ diseñado para encontrar un conjunto óptimo de métodos generadores de objectos en una API. 
...
junto con un valor lógico que indica que se deben considerar únicamente constructores. 
Posteriormente, el algoritmo invoca nuevamente a $hillClimbin())$ para continuar la búsqueda incluyendo los otros métodos de la API con el mejor constructor que obtuvo
...
Entre estos sucesores se selecciona el que presente el mejor valor según la función de aptitud $f()$. Si dicho sucesor no mejora el valor del estado actual,
...
Para ello, recorre la lista evaluando el valor de la función de aptitud ($f()$)en cada elemento y selecciona aquel que tenga el mayor valor. (línea 30 a 38)
...
dado que el algoritmo solo avanza hacia soluciones que mejoran directamente la aptitud del estado actual. 
...
%  \pp{Habría que hacer un algoritmo con esto.}. 
...
%  \pp{Esto está al revés, hay que explicar esto primero en el algoritmo, y después lo del proceso iterativo que está en el párrafo de arriba.}
