...
\vspace{5pt} 
    ...
    (7)  l.addFirst(-1);
    \end{lstlisting}
\pp{Secuencia de test incompleta, falta la lista sobre la que se aplica}
\cacho{Listo todo esto}
...
\emph{Infer} ofrece funcionalidades avanzadas para analizar código fuente y, en
...
Si bien \emph{Infer} puede generar falsos negativos —no clasificar como
...
herramienta \emph{Randoop} (descrita en la sección \ref{sec:feedback-directed-test-gen})
...
\ref{sec:feedback-directed-test-gen}, basada en el enfoque de Randoop. \pp{Otra
vez mucho espacio antes del un algoritmo, casi una página en blanco. Quitar este
espacio!} \cacho{resuelto}
...
asignando un peso de 0.4 a las líneas y de 0.6 a las ramas. \pp{según la fórmula anterior es 0.4 * lineas y 0.6 por
ramas. Corregir los números en este párrafo.}. \cacho{Modifique este parrafo }
...
%
Como el conjunto \( M_1 \) logra una mejor cobertura, y por lo tanto:
...
escrita en Java, llamada \emph{Jenetics}\footnote{\url{https://jenetics.io/}}
...
\emph{Jenetics} es una biblioteca robusta y versátil que ofrece una amplia gama
de funcionalidades para la implementación de algoritmos genéticos. Además,
\emph{Jenetics} es muy fácil de usar, con una documentación completa y una
comunidad activa de usuarios que proporciona soporte y ayuda. Una característica
relevante de \emph{Jenetics} es que permite realizar la evaluación de la
    ...
    for (int i = 0; i < maxLength; i++) {
        switch (Verify.getInt(n_methods - 1)) {
            ...
            case 13:
                ...
                break;
            case 14:
                t.headMap(Verify.getInt(0,scope));
                break;
            case 15:
                t.tailMap(Verify.getInt(0,scope));
                break;
            case 16:
                t.descendingMap();
                break;
...
\end{lstlisting}
\end{minipage}
\pp{Por qué los puntos suspensivos? Habría que poner un comentario que diga
    métodos omitidos o algo así. Se corta la figura. Mismo problema que en la
sección de preliminares de JPF.}
\cacho{Agregue el ultimo y el comentario}
