...
\cacho{No estoy seguro si esto ya lo llevo a preliminates, no tengo NCL ahi}
...
% aca Verify.random(int n) \cacho{PAra que son}
...
En el caso en que la persona usuaria no conozca el conjunto de
\emph{builders} (y no desee realizar la difícil tarea de seleccionarlos
...
Si bien evitar este crecimiento exponencial no es posible en muchos casos, en este trabajo se propone utilizar sólo los builders detectados por el algoritmo
...
genera exactamente los mismos objetos de NodeCachingLinkedList que el anterior (porque los builders son suficientes y minimales), y por lo tanto JPF explora
...
\begin{lstlisting}[caption={Probando la propiedad \ref{alg:propiedad-a-chequear} con JPF},label={lst:driverNCLBuilders},language=Java,captionpos=b]
