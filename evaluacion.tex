...
En esta sección, evaluamos experimentalmente nuestros enfoques presentados en el capítulo \ref{cap:builders}. Describiremos como es la eficiencia de cada algoritmo con las funciones de valoración presentada en la seccion \ref{sec:fitness}.
...
Para corroborar nuestros resultados, necesitamos conocer cuales son los metodos builders minimos y suficiente de cada caso de estudio. Para esto se creo un \emph{ground Truth} manualmente. Para esto hemos inspeccionado cada caso de manera manual. Esto llevo un largo trabajo para estudiar los casos de estudio mas complejo. El \emph{ground truth} de nuestros casos de estudios se pueden observar en la tabla \ref{tab:groundTruth}. Esta incluye el número de rutinas en la API completa (\#API), y el conjunt de los \emph{builders} identificados manualmente (algunos métodos pueden intercambiarse en diferentes ejecuciones, por ejemplo, \texttt{addFirst} y \texttt{addLast} en NCL)
...
\subsection{Efectivdad}
La efeciencia de nuestro algoritmos es la cantidad de segundos que le lleva a cada uno de nuestros algoritmos con cada funcion de valoracion para encontrar el subconjunto de metodos \emph{builders} sufientes. Para evaluar la eficiencia de nuestro algoritmo, llevamos a cabo experimentos utilizando conjuntos de datos de muestra y medimos el tiempo de promedio de las 10 ejecuciones necesarias para obtener las soluciones. Comparando los resultados obtenidos con la técnica evolutiva, la técnica greedy y la tecnica de clase de equivalencia, pudimos evaluar la eficiencia de ambas enfoques y que eficicia de acuerdo a la funcion de valoracion utilizadas y determinar cuál es más rápido. Estos resultados se pueden ver en \ref{tab:eficiencia}.

\input{tables/compareAllBuilders}

\subsection{Efectividad}

La efectividad de nuestros algoritmos se basa en cuan lejos esta de encontrar el conjunto de metodos \emph{builders}  que sean minimos. Vale aclarar que nuestros algoritmos siempre encuentran al menos el subconjunto de metodos \emph{builders} que son suficientes. Pero cuanto mas minimo sea este subconjuntos mejores resultados vamos a obtener a la hora de utilizarlos, en por ejemplo, durante la generacion de inputs para testing.
Para evaluar la efectividad de nuestro algoritmo, realizamos experimentos utilizando las estrucuturas de datos especificadas en \ref{tab:groundTruth}. Recuerde que el ground truth proporciona la solución óptima para comparar con los resultados obtenidos por los algoritmos. Hemos realizado 10 ejecuciones del algrotimos bajo la misma configuracion de parametros. Luego chequeamos cuantos metodos de mas tiene el subconjunto hallado con respecto a los metodos minimos para esa estructura de dato.
